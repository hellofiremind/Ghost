 ...
 * @param {Object} transaction.knex - knex instance
 */
async function addForeign(fromTable, fromColumn, toTable, toColumn, transaction) {
    ...
    if (!hasForeignKey) {
        ...
        logging.warn(`Foreign key for ${fromColumn} in ${fromTable} to ${toColumn} in ${toTable} already exists`);
    }
}
...
async function dropForeign(fromTable, fromColumn, toTable, toColumn, transaction) {
    ...
    if (hasForeignKey) {
        ...
        logging.warn(`Dropping key for ${fromColumn} in ${fromTable} to ${toColumn} in ${toTable} does not exist`);
    }
}
